...
class Gamepad:
    ...
    def toggle_buttons(self, *buttons):
        """Toggle the given buttons."""
        for button in buttons:
            self._buttons_state ^= ~(1 << self._validate_button_number(button) - 1)
        self._send()

    def get_button(self, button):
        """Get the value of a button, true = pressed and false = not pressed"""
        return (self._buttons_state >> (self._validate_button_number(button) - 1) & 1) == 1
